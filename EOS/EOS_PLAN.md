...
```
Replicate to all config variant files (config.standard.json, config.full.json, etc.) with appropriate tuning per mode.
...
**File(s):** `config.json`, `config.standard.json`, `config.full.json`, `config.base.json`, all variants
